...
genderlabour <- subset(genderdf, genderdf$category == 'labour_force_participation_rate')
unique(genderlabour$subgroup)

unique(genderdf$category)


gendertest <- subset(genderdf, genderdf$category %in% c("total_fertility_rate, adult_literacy_rate", "abortion_rate", "womens_share_of_labour_force", "womens_share_of_tertiary_enrolment", "labour_force_participation_rate", "percent_ever_married_or_in_union_among_persons_aged_15_19", "singulate_mean_age_at_marriage", "contraceptive_prevalence_rate_any_method"))

head(gendertest)

testing <- subset(gendertest, gendertest$category == 'abortion_rate')
unique(testing$subgroup)
names(testing)
unique(testing$unit)

# Select abortion rate values, make them into a vector, and add them as a new column in the dataframe testing
abortionrate <- subset(gendertest, gendertest$category == 'abortion_rate', select = 'value')
abortionrate <- c(abortionrate$value, rep(NA, nrow(gendertest) - nrow(abortionrate)))
gendertest$abortion_rate <- abortionrate
